...
async function generatePremiumResponse(message: string, conversationHistory: any[], propertyAddress?: string, projectDescription?: string) {
  ...
  // Automatically research property data if address is provided
  let propertyResearchData = null;
  if (propertyAddress) {
    try {
      console.log(`Researching comprehensive property data for: ${propertyAddress}`);
      const { researchProperty } = await import('./property-research');
      propertyResearchData = await researchProperty(propertyAddress);
      console.log('Property research completed:', propertyResearchData);
    } catch (error) {

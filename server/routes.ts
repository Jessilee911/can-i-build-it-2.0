...
async function generatePlanBasedResponse(message: string, plan: string, conversationHistory: any[]) {
  ...
  const systemPrompts = {
    ...
    - Wind and earthquake zone considerations
  ...
  const systemPrompt = systemPrompts[plan as keyof typeof systemPrompts] || 
    `You are an AI property advisor. The user doesn't have an active plan. Provide general information and suggest upgrading for detailed advice.`;

  // Enhanced query for RAG system
  const enhancedQuery = `${systemPrompt}\n\nConversation context:\n${context}\n\nUser message: ${message}\n\nProvide a helpful response based on the user's ${plan} plan level.`;
  ...
  try {
    ...
    return enhancePlanResponse(response, plan);
  } catch (error) {
    console.error('Error generating plan-based response:', error);
    return getFallbackResponse(plan);
  }
}
...
function getFallbackResponse(plan: string) {
  switch (plan) {
    case 'basic':
      return "I'm here to help with your basic property questions. Could you tell me more about what specific aspect of your property development you'd like guidance on?";
    
    case 'professional':
      return "I'd be happy to provide detailed professional guidance. Could you share more details about your property and development plans?";
    
    case 'premium':
      return "With your Premium plan, I can provide comprehensive analysis. Let me know your property details and development goals for expert guidance.";
    
    default:
      return "I can provide general building guidance. For detailed property analysis, please consider selecting one of our plans.";
  }
}

...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ==================== Chat Routes ====================
  // Main Chat Agent (Agent 1) - General guidance
  apiRouter.post("/api/main-chat", async (req: Request, res: Response) => {
    try {
      ...
      const response = await generateMainChatResponse(message, conversationHistory);
      res.json({ response });
    } catch (error: any) {
  ...
  // Property Assessment Agent (Agent 2) - Detailed property analysis
  apiRouter.post("/api/chat", async (req: Request, res: Response) => {
    try {
      ...
      const response = await generatePropertyChatResponse(message, conversationHistory);
      res.json({ response });
    } catch (error: any) {
...
// Main Chat Agent (Agent 1) - General guidance and FAQ
async function generateMainChatResponse(message: string, conversationHistory: any[]) {
  const context = conversationHistory.map(msg => `${msg.type}: ${msg.content}`).join('\n');
  
  const systemPrompt = `You are Agent 1, a friendly general assistant for New Zealand building regulations. You provide:
    
    - General information about NZ building codes and consent processes
    - Overview of resource consent requirements
    - Basic planning and zoning explanations
    - Direction to specialized resources and tools
    - Helpful guidance for getting started with property development
    
    Keep responses informative but general. If users need detailed property-specific analysis, suggest they use Agent 2 (Property Assessment Agent) which specializes in site-specific guidance.
    
    Be friendly and supportive. Use simple language and explain concepts clearly.`;
  ...
  try {
    ...
    if (message.toLowerCase().includes('property') || message.toLowerCase().includes('specific') || message.toLowerCase().includes('address')) {
      return response + "\n\n💡 For detailed property-specific analysis, try our Property Assessment Agent (Agent 2) which can provide site-specific guidance tailored to your exact location.";
    }
    
    return response;
  } catch (error) {
    console.error('Error generating main chat response:', error);
    return "I'm here to help with general New Zealand building regulation questions. What would you like to know about building consents, resource consents, or planning requirements?";
  }
}

// Property Assessment Agent (Agent 2) - Detailed property analysis
async function generatePropertyChatResponse(message: string, conversationHistory: any[]) {
  const context = conversationHistory.map(msg => `${msg.type}: ${msg.content}`).join('\n');
  
  const systemPrompt = `You are Agent 2, a specialized property assessment advisor for New Zealand. You focus on:
    
    - Property-specific building consent analysis and requirements
    - Detailed development potential assessment for specific sites
    - Site-specific constraints, opportunities, and compliance issues
    - Zoning compliance analysis for exact locations
    - Detailed cost estimates and project timelines
    - Step-by-step development guidance tailored to specific properties
    - Technical building code interpretations for specific projects
    
    Provide thorough, detailed analysis. Ask for specific property addresses when needed.
    Focus on actionable, property-specific advice rather than general information.
    
    If users need comprehensive written documentation, suggest the "Generate Report" feature.`;

  const enhancedQuery = `${systemPrompt}\n\nConversation context:\n${context}\n\nUser message: ${message}\n\nProvide detailed property-specific guidance.`;

  try {
    const response = await generateRAGResponse(enhancedQuery);
    
    // Add property-specific enhancements
    if (!message.toLowerCase().includes('address') && !context.includes('address')) {
      return response + "\n\n🏠 For the most accurate guidance, please share your property address so I can provide location-specific advice about zoning, consents, and development potential.";
    }
    
    return response;
  } catch (error) {
    console.error('Error generating property chat response:', error);
    return "I'm here to provide detailed property assessment guidance. Please share your property address and development plans so I can give you specific advice about building consents, zoning compliance, and development opportunities.";
  }
}

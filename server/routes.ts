...
• Complex projects may require additional time for engineering review`;
  ...
  // ==================== Stats Routes ====================
  // AR Zoning Overlay API endpoint
  apiRouter.post("/api/ar-zoning", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`AR Zoning request for coordinates: ${latitude}, ${longitude}`);

      // Query Auckland Council API for zoning and overlay data
      const { aucklandCouncilAPI } = await import('./auckland-council-api');
      const propertyData = await aucklandCouncilAPI.queryPropertyDatasets(latitude, longitude, 'AR Location');
      ...
      const arZoningData = {
        ...
        accuracy: calculateLocationAccuracy(latitude, longitude)
      };

      res.json(arZoningData);
    } catch (error) {
      console.error('AR Zoning API error:', error);
      res.status(500).json({ error: 'Failed to retrieve zoning data' });
    }
  });

  // Helper function to extract overlay restrictions for AR display
  function getOverlayRestrictions(overlay: any): string[] {
    const restrictions: string[] = [];
    
    switch (overlay.type) {
      case 'special_character_areas':
        restrictions.push('Character preservation required');
        restrictions.push('Design review may be needed');
        restrictions.push('Heritage architectural features must be maintained');
        break;
      case 'heritage_overlay':
        restrictions.push('Heritage protection applies');
        restrictions.push('Demolition restrictions');
        restrictions.push('Material and design constraints');
        break;
      case 'museum_viewshaft':
      case 'stockade_hill_viewshaft':
        restrictions.push('Height restrictions apply');
        restrictions.push('View corridor protection');
        restrictions.push('Building bulk limitations');
        break;
      case 'notable_trees':
        restrictions.push('Tree protection rules');
        restrictions.push('Root zone restrictions');
        restrictions.push('Canopy preservation');
        break;
      case 'flood_sensitive_areas':
        restrictions.push('Flood mitigation required');
        restrictions.push('Floor level restrictions');
        restrictions.push('Drainage requirements');
        break;
      case 'liquefaction_vulnerability':
        const vulnerability = overlay.data.Vulnerability || 'Unknown';
        restrictions.push(`Liquefaction risk: ${vulnerability}`);
        if (vulnerability !== 'Very Low') {
          restrictions.push('Geotechnical assessment may be required');
        }
        break;
      default:
        restrictions.push('Additional planning requirements apply');
    }
  ...
  function calculateLocationAccuracy(lat: number, lon: number): number {
    // Base accuracy estimate for GPS coordinates
    return Math.floor(Math.random() * 10) + 5; // 5-15 meter range
  }
